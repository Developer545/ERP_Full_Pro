...
export const THEMES: AppTheme[] = [
  // ──────────────────────────────────────────────────────
  // 1. Speeddansys Orange Dark (predeterminado)
  // ──────────────────────────────────────────────────────
  {
    id: "speeddansys-orange",
    name: "Speeddansys Orange",
    emoji: "🔥",
 ...
 * Permite que cualquier CSS use var(--theme-*) sin hardcodear colores.
 */
export function applyThemeCSSVars(
  ...
  root.style.setProperty(
    "--theme-menu-selected-bg",
    appTheme.menuItemSelectedBg
  );
  ...
  root.style.setProperty(
    "--theme-menu-selected-color",
    appTheme.menuItemSelectedColor
  );
  root.style.setProperty("--theme-border", appTheme.borderColor);
  root.style.setProperty("--theme-shadow", appTheme.boxShadow);

  // Scrollbar adaptativa
  root.style.setProperty(
    "--scrollbar-thumb",
    appTheme.isDark ? "rgba(255,255,255,0.15)" : "rgba(0,0,0,0.18)"
  );
  root.style.setProperty(
    "--scrollbar-track",
    appTheme.isDark ? "rgba(255,255,255,0.04)" : "rgba(0,0,0,0.04)"
  );

  // Atributo para activar reglas CSS por modo
  root.setAttribute("data-theme", appTheme.isDark ? "dark" : "light");
}
...
/**
 * Genera la configuracion de Ant Design 6 a partir del tema activo.
 */
export function buildAntThemeConfig(
  appTheme: AppTheme,
  customColor?: string
): ThemeConfig {
  const primary =
  ...
  return {
    ...
    components: {
      ...
      Table: {
        headerBg: appTheme.isDark ? appTheme.headerBg : "#fafafa",
        rowHoverBg: appTheme.isDark

...
export default function ThemeSelector({ open, onClose }: ThemeSelectorProps) {
  const { themeId, customColor, setThemeId, setCustomColor } = useThemeStore();
  ...
  const handleHexChange = (val: string) => {
    ...
    setHexInput(clean);
    if (/^#[0-9a-fA-F]{6}$/.test(clean)) setPendingColor(clean);
  };
  ...
  const activeColor =
    pendingColor && /^#[0-9a-fA-F]{6}$/.test(pendingColor)
      ? pendingColor
      : pendingTheme?.colorPrimary ?? "#f47920";
  ...
  return (
    ...
    >
      ...
      >
        Reemplaza el color primario del tema. Deja en blanco para usar el color
        original.
      </Text>
            ...
            <Tooltip key={color} title={`${label} — ${color}`}>
              <div
                ...
                style={{
                  ...
                  transform:
                    pendingColor === color ? "scale(1.15)" : "scale(1)",
                }}
              />
            </Tooltip>
          ))}
        ...
        {pendingTheme && (
          <div
            style={{
              ...
              padding: "10px 14px",
              borderRadius: 8,
              background: pendingTheme.isDark
...
function ThemeCard({ theme, isSelected, onSelect }: ThemeCardProps) {
  return (
    ...
    >
          ...
          {[0.6, 0.35, 0.25].map((op, i) => (
            <div
              ...
              style={{
                height: 4,
                borderRadius: 2,
                background: theme.menuItemColor,
                opacity: op,
              }}
            />
          ))}
      ...
      {/* Nombre + barra de color */}
      <div style={{ padding: "6px 8px 7px" }}>
        <div
          style={{
            fontSize: 11,
            fontWeight: 600,
            color: theme.isDark ? "#e0e0e0" : "#222",
            whiteSpace: "nowrap",
            overflow: "hidden",
